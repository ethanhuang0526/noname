...
(async function () {
	...
	const globalText = {
		...
		REDIRECT_TIP: [
			"您使用的浏览器或无名杀客户端内核版本过低，已经无法正常运行无名杀！",
			"目前使用的浏览器UA信息为: ",
			userAgent,
	...
	if (!localStorage.getItem("gplv3_noname_alerted")) {
		...
		// 判断游戏是否已经初始化过
		const gameIntialized =
			nonameInitialized && nonameInitialized.length > 0;

		// 如果满足以下条件之一，则显示GPL许可协议警告:
		// 1. 游戏运行在HTTP环境中且已经初始化过
		// 2. 用户确认显示GPL许可协议警告
		if (
			(gameInHttpEnvironment && gameIntialized) ||
			confirm(globalText.GPL_ALERT)
		) {
			// 记录已显示过GPL许可协议警告
	...
	if (userAgent.includes("safari") && !userAgent.includes("chrome")) {
		...
		if (
			coreName === "safari" &&
			!get.checkVersion(minSafariVersion, safariVersion)
		) {
	...
	if (location.protocol.startsWith("http") && "serviceWorker" in navigator) {
		...
		try {
			...
			registration_1
				.update()
				.catch((e) => console.error("worker update失败", e));
			...
			tryUpdateClient(type, text = "") {
				...
				function update(url, { request, createProgress }) {
					...
					return (
						request(url, (receivedBytes, total, filename) => {
							...
							received = +(receivedBytes / (1024 * 1024)).toFixed(
								1
							);
				...
				function fallback(text) {
					...
					if (confirm(text)) {
						...
						window.open(
							"https://github.com/libccy/noname/releases/tag/chromium85-client"
						);
					}
				}
				...
				switch (type) {
					case UpdateReason.DEBUG: {
						// 测试环境
						let url =
							"https://ghproxy.cc/https://github.com/libccy/noname/releases/download/chromium85-client/Noname-linux-x64.zip";
					...
					case UpdateReason.UNDERSUPPORT: {
						...
						if (androidNewStandardApp) {
							let tips = [
								"您使用的无名杀客户端已达到最新，但目前的浏览器内核版本过低，未来可能将无法使用！",
								"目前使用的浏览器UA信息为: ",
								userAgent,
								"新版本的客户端在机器存在Chrome的情况下会直接使用Chrome的内核",
								"请前往下载最新版的Chrome，以获取最佳的体验！",
								"稍后游戏将继续正常运行，但我们不保证不会出现任何报错",
							].join("\n");

							let packageName =
								window.NonameAndroidBridge.getPackageName();
							if (
								!(
									packageName.includes("shijian") ||
									packageName.includes("yuri")
								)
							) {
								tips = [
									"您使用的无名杀客户端已达到最新，但目前的浏览器内核版本过低，未来可能将无法使用！",
									"目前使用的浏览器UA信息为: ",
									userAgent,
									"检测到你现在使用的是第三方客户端，请联系客户端制作者寻求帮助！",
									"稍后游戏将继续正常运行，但我们不保证不会出现任何报错",
								].join("\n");
							}
						...
						// 此时将先考虑能不能加载更新代码
							...
							let tips = [
								"你使用的无名杀客户端版本号未达到最新无名杀需要的要求，未来可能将无法正常运行无名杀！",
								"目前使用的浏览器UA信息为: ",
								userAgent,
								"如果你使用的是第三方客户端，请联系客户端制作者更新或寻求解决方法！",
								"点击“确认”将开始下载最新版客户端（如果你使用的是第三方客户端，请不要点击“确认”）",
								"稍后游戏将继续正常运行，但我们不保证不会出现任何报错",
							].join("\n");
							let fallbacks = [
								"你使用的无名杀客户端版本号未达到最新无名杀需要的要求，已无法正常运行无名杀！",
								"目前使用的浏览器UA信息为: ",
								userAgent,
								"如果你使用的是第三方客户端，请联系客户端制作者更新或寻求解决方法！",
							...
							function callback(module) {
								...
								if (confirm(tips)) {
									let url =
										"https://ghproxy.cc/https://github.com/libccy/noname/releases/download/chromium85-client/Noname-yuri-v1.9.2.apk";

									if (coreVersion == 77) {
										let compatibleTips = [
											"检测到你现在的版本号为上版本兼容版的版本号，由于当前版本无法确认是否为兼容版，特此在此再次询问",
											"请问你是否需要下载最新的兼容版？",
											"（目前由理版可直接使用已安装的Chrome内核，但如果无法安装最新的Chrome，依然需要兼容版）",
										].join("\n");
										if (confirm(compatibleTips)) {
											url =
												"https://ghproxy.cc/https://github.com/libccy/noname/releases/download/chromium85-client/Noname-yuri-compatible-v1.8.3.apk";
										}
									}
							...
							needToWait = import("../library/update.js").then(
								callback,
								() => fallback(fallbacks)
							);
						}
						...
						// 使用chrome的，直接提示更新（不是现在还有人用Chrome 85以下的版本吗）
							...
							let tips = [
								"你使用的浏览器内核已无法达到无名杀的最低要求，未来可能将无法使用！",
								"请更新你的Google Chrome/Chromium内核！",
								"稍后游戏将继续正常运行，但我们不保证不会出现任何报错",
							].join("\n");
							alert(tips);
						}
